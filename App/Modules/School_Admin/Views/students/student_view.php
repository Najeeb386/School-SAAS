    ...
    fetch('get_student.php?id=' + STUDENT_ID, { credentials: 'same-origin' })
      ...
      .then(data => {
        ...
        if (academics.length === 0) {
          ...
          const a = academics[0];
          html += '<div class="row">';
          html += '<div class="col-md-4"><div class="kv">Session</div><div>'+(a.session_label||a.session_id||'-')+'</div></div>';
          html += '<div class="col-md-4"><div class="kv">Class</div><div>'+(a.class_name||a.class_id||'-')+'</div></div>';
          html += '<div class="col-md-4"><div class="kv">Section</div><div>'+(a.section_name||a.section_id||'-')+'</div></div>';
          html += '</div>';
        }
        html += '</div></div>';
